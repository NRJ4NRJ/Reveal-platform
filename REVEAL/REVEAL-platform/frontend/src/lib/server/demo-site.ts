/**
 * Demo site — "REVEAL" (based on the Pruny solar farm template).
 * Every new user is automatically connected to this site on registration.
 * Existing users can be backfilled via `prisma/backfill-demo-site.ts`.
 */
import { prisma } from "@/lib/prisma";

export const DEMO_SITE_ID = "REVEAL_DEMO_SITE";
...
/**
 * Ensures the shared REVEAL demo site exists in the database, then connects
 * the given user to it. Safe to call multiple times — fully idempotent.
 */
export async function ensureDemoSiteForUser(userId: string): Promise<void> {
  ...
  // Connect the user to the demo site if not already connected
