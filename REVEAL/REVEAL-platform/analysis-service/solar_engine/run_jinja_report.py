...
Loads data via repat_solar_scada_analysis, runs the full analysis pipeline,
then generates HTML + PDF via the report/ Jinja2 template pipeline
(chart_factory → build_report_data → render_report).

Output: _report_test_output/REPAT_SCADA_Analysis_Report.html + .pdf
"""
...
import repat_solar_scada_analysis as _pv

# ── Output paths ─────────────────────────────────────────────
DATA_DIR    = _pv.DATA_DIR
OUT_DIR     = _pv.OUT_DIR
ASSETS_DIR  = OUT_DIR / "REPAT_SCADA_Analysis_Report_assets"
REPORT_STEM = "REPAT_SCADA_Analysis_Report"
