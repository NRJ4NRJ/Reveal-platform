...
"""
REPAT Solar SCADA Analysis Tool
...
Output: REPAT_SCADA_Analysis_Report.pdf
"""
...
DEFAULT_DATA_DIR = SCADA_PV_ROOT / "00orig"
DEFAULT_OUT_DIR  = SCADA_PV_ROOT / "_report_test_output"
DEFAULT_REPORT   = "REPAT_SCADA_Analysis_Report.pdf"
...
def page_cover(pdf):
    ...
    ax.add_patch(plt.Rectangle((0, 0.068), 1, 0.003,
                                facecolor=C['orange'], edgecolor='none', zorder=9))
    ax.text(0.04, 0.034, 'REPAT / 8.2 Advisory | A Dolfines Company',
...
def parse_args():
    parser = argparse.ArgumentParser(description="REPAT solar SCADA analysis and PDF report generation")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path(os.getenv("REPAT_DATA_DIR", str(DEFAULT_DATA_DIR))),
        help="Input data folder containing CSV inputs and assets",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(os.getenv("REPAT_OUT_DIR", str(DEFAULT_OUT_DIR))),
        help="Output folder for PDF report and run manifest",
    )
    parser.add_argument(
        "--report-name",
        type=str,
        default=os.getenv("REPAT_REPORT_NAME", DEFAULT_REPORT),
...
def main():
    ...
    print("=" * 65)
    print("  REPAT Solar SCADA Analysis Tool  -  Solar PV Performance Report")
    print("=" * 65)
    ...
    with PdfPages(out_path) as pdf:
        ...
        d['Author']       = 'REPAT Analysis Tool | Dolfines'
